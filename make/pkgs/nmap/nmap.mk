$(call PKG_INIT_BIN, $(if $(FREETZ_PACKAGE_NMAP_VERSION_4),4.76,$(if $(FREETZ_PACKAGE_NMAP_VERSION_5),5.51,7.991)))
$(PKG)_SOURCE:=$(pkg)-$($(PKG)_VERSION).tar.bz2
$(PKG)_HASH_4:=4e24328ca6ec97afb2a8caf312b1f111a15cf41763a5ac41e7a633fdb217d66d
$(PKG)_HASH_5:=15b3e134a74fa9b54aba2c3a53d6701c7ad221dc4051657ef95465a5a5a8687e
$(PKG)_HASH_7:=a5d507f29437bef3bedd4771ff9aaa8fc1c2a109ddba1f5b1cf12027456929be
$(PKG)_HASH:=$($(PKG)_HASH_$(call GET_MAJOR_VERSION,$($(PKG)_VERSION),1))
$(PKG)_SITE:=https://nmap.org/dist
### WEBSITE:=https://nmap.org/
### MANPAGE:=https://nmap.org/docs.html
### CHANGES:=https://nmap.org/changelog.html
### CVSREPO:=https://github.com/nmap/nmap
### STEWARD:=fda77

ifneq ($(strip $(FREETZ_PACKAGE_NMAP_VERSION_4)),y)
$(PKG)_BINARIES5_ALL       := ncat nping
$(PKG)_BINARIES5           := $(call PKG_SELECTED_SUBOPTIONS,$($(PKG)_BINARIES5_ALL))
$(PKG)_BINARIES5_BUILD_DIR := $(addprefix $($(PKG)_DIR)/, $(join $($(PKG)_BINARIES5), $(addprefix /,$($(PKG)_BINARIES5))))
endif
$(PKG)_BINARIES_ALL        := nmap $($(PKG)_BINARIES5_ALL)
$(PKG)_BINARIES            := nmap $($(PKG)_BINARIES5)
$(PKG)_BINARIES_BUILD_DIR  := $($(PKG)_DIR)/nmap $($(PKG)_BINARIES5_BUILD_DIR)

$(PKG)_BINARIES_TARGET_DIR := $(addprefix $($(PKG)_DEST_DIR)/usr/bin/,$($(PKG)_BINARIES))

$(PKG)_DATADIR        := /usr/share/nmap

$(PKG)_DBS_ALL        := mac-prefixes os-db payloads protocols rpc service-probes services
$(PKG)_DBS            := $(call PKG_SELECTED_SUBOPTIONS,$($(PKG)_DBS_ALL))

$(PKG)_DBS_BUILD_DIR  := $($(PKG)_DBS:%=$($(PKG)_DIR)/nmap-%)
$(PKG)_DBS_TARGET_DIR := $($(PKG)_DBS:%=$($(PKG)_DEST_DIR)$($(PKG)_DATADIR)/nmap-%)

$(PKG)_EXCLUDED       += $(patsubst %,$($(PKG)_DEST_DIR)/usr/bin/%,$(filter-out $($(PKG)_BINARIES),$($(PKG)_BINARIES_ALL)))
$(PKG)_EXCLUDED       += $(patsubst %,$($(PKG)_DEST_DIR)$($(PKG)_DATADIR)/nmap-%,$(filter-out $($(PKG)_DBS),$($(PKG)_DBS_ALL)))

$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_VERSION_4
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_VERSION_5
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_VERSION_7
$(PKG)_REBUILD_SUBOPTS += FREETZ_STDCXXLIB
$(PKG)_REBUILD_SUBOPTS += FREETZ_LIB_liblua_WITH_VERSION_ABANDON
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_DNET
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_PCAP
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE2
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_OPENSSL
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_Z
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_SSH2
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_WITH_SHARED_LINEAR
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_NMAP_STATIC
$(PKG)_REBUILD_SUBOPTS += FREETZ_TARGET_IPV6_SUPPORT

$(PKG)_DEPENDS_ON += config-host
$(PKG)_DEPENDS_ON += $(STDCXXLIB)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_DNET),libdnet)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA),lua)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCAP),libpcap)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE),pcre)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE2),pcre2)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_OPENSSL),openssl)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_Z),zlib)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_SSH2),libssh2)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LINEAR),liblinear)

$(PKG)_CONDITIONAL_PATCHES+=$(call GET_MAJOR_VERSION,$($(PKG)_VERSION),1)

# make sure nmap never fallbacks to using the bundled libraries by deleting them
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_DNET),$(RM) -r libdnet-stripped;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA),$(RM) -r liblua;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCAP),$(RM) -r libpcap;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE),$(RM) -r libpcre;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE2),$(RM) -r libpcre;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_Z),$(RM) -r libz;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_SSH2),$(RM) -r libssh2;)
$(PKG)_PATCH_POST_CMDS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LINEAR),$(RM) -r liblinear;)

ifeq ($(strip $(FREETZ_PACKAGE_NMAP_VERSION_4)),y)
$(PKG)_CONFIGURE_PRE_CMDS += $(call PKG_UPDATE_CONFIGS,./)
endif
ifeq ($(strip $(FREETZ_PACKAGE_NMAP_VERSION_5)),y)
$(PKG)_CONFIGURE_PRE_CMDS += $(call PKG_UPDATE_CONFIGS,./)
$(PKG)_CONFIGURE_PRE_CMDS += $(call PKG_UPDATE_CONFIGS,./nping/)
$(PKG)_CONFIGURE_PRE_CMDS += $(call PKG_UPDATE_CONFIGS,./ncat/)
endif

$(PKG)_CONFIGURE_ENV += ac_cv_header_lua_h=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA),yes,no)
ifeq ($(strip $(FREETZ_PACKAGE_NMAP_VERSION_4)),y)
ifeq ($(strip $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA)),y)
$(PKG)_CONFIGURE_ENV += LUAINCLUDE=-I"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr/include/lua"
endif
$(PKG)_CONFIGURE_OPTIONS += --with-nmapfe=no
else
$(PKG)_CONFIGURE_OPTIONS += --with-ncat
$(PKG)_CONFIGURE_OPTIONS += --with-nping
$(PKG)_CONFIGURE_OPTIONS += --without-ndiff
endif

$(PKG)_CONFIGURE_OPTIONS += --without-zenmap

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_TARGET_IPV6_SUPPORT),--enable-ipv6,--disable-ipv6)
$(PKG)_CONFIGURE_OPTIONS += --with-libdnet=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_DNET),"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",included)
$(PKG)_CONFIGURE_OPTIONS += --with-liblua=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LUA),"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",included)
$(PKG)_CONFIGURE_OPTIONS += --with-libpcap=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCAP),"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",included)
# ssl support requires openssl built with ripemd enabled
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_OPENSSL),--with-openssl="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--without-openssl)

ifneq ($(strip $(FREETZ_PACKAGE_NMAP_VERSION_7)),y)
$(PKG)_CONFIGURE_OPTIONS += --with-libpcre=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE),"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",included)
else
$(PKG)_CONFIGURE_OPTIONS += --with-libpcre=$(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_PCRE2),"$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",included)
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_Z),--with-libz="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--without-libz)
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_SSH2),--with-libssh2="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--without-libssh2)
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_NMAP_WITH_SHARED_LINEAR),--with-libz="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr",--with-liblinear=included)
endif

# libnbase & libnsock:
# no extra configure options are required in order to use bundled versions of these libraries.


$(PKG_SOURCE_DOWNLOAD)
$(PKG_UNPACKED)
$(PKG_CONFIGURED_CONFIGURE)

$($(PKG)_BINARIES_BUILD_DIR): $($(PKG)_DIR)/.configured
	$(SUBMAKE1) -C $(NMAP_DIR) \
		$(if $(FREETZ_PACKAGE_NMAP_STATIC),STATIC="-static")

$(foreach binary,$($(PKG)_BINARIES_BUILD_DIR),$(eval $(call INSTALL_BINARY_STRIP_RULE,$(binary),/usr/bin)))

$($(PKG)_DBS_BUILD_DIR): $($(PKG)_DIR)/.configured
	touch $@

$($(PKG)_DBS_TARGET_DIR): $($(PKG)_DEST_DIR)$($(PKG)_DATADIR)/nmap-%: $($(PKG)_DIR)/nmap-%
	$(INSTALL_FILE)

$(pkg):

$(pkg)-precompiled: $($(PKG)_BINARIES_TARGET_DIR) $($(PKG)_DBS_TARGET_DIR)


$(pkg)-clean:
	-$(SUBMAKE) -C $(NMAP_DIR) clean
	$(RM) $(NMAP_DIR)/.configured

$(pkg)-uninstall:
	$(RM) -r $(NMAP_BINARIES_ALL:%=$(NMAP_DEST_DIR)/usr/bin/%) $(NMAP_DEST_DIR)$(NMAP_DATADIR)

$(PKG_FINISH)
