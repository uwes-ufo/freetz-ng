...
ifeq ($(strip $(FREETZ_PACKAGE_PHP5_STATIC)),y)
...
$(PKG)_EXCLUDED += $(if $(FREETZ_PACKAGE_PHP5_cli),,$($(PKG)_CLI_TARGET_BINARY))

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_apxs2)),y)
$(PKG)_DEPENDS_ON += apache2
$(PKG)_CONFIGURE_OPTIONS += --with-apxs2="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr/bin/apxs"
endif

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_CURL)),y)
...
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_FILEINFO),--enable-fileinfo,--disable-fileinfo)

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_FTP),--enable-ftp,--disable-ftp)

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_GD)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_GETTEXT)),y)
$(PKG)_DEPENDS_ON += gettext
$(PKG)_CONFIGURE_OPTIONS += --with-gettext="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr"
endif

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_ICONV)),y)
$(PKG)_CONFIGURE_OPTIONS += --with-iconv

$(PKG)_CONFIGURE_ENV += ac_cv_func_iconv=$(if $(FREETZ_PACKAGE_PHP5_WITH_LIBICONV),no,yes)
$(PKG)_CONFIGURE_ENV += ac_cv_func_libiconv=$(if $(FREETZ_PACKAGE_PHP5_WITH_LIBICONV),yes,no)

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_LIBICONV)),y)
...
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_JSON),--enable-json,--disable-json)

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_LIBXML)),y)
...
$(PKG)_XML_SUPPORT:=$(if $(FREETZ_PACKAGE_PHP5_WITH_LIBXML),enable,disable)
...
$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_MHASH),--with-mhash,--without-mhash)

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_MEMORY_LIMIT),--enable-memory-limit,--disable-memory-limit)

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_PCNTL),--enable-pcntl,--disable-pcntl)

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_SESSION),--enable-session,--disable-session)

$(PKG)_CONFIGURE_OPTIONS += $(if $(FREETZ_PACKAGE_PHP5_WITH_SOCKETS),--enable-sockets,--disable-sockets)

$(PKG)_CONFIGURE_OPTIONS += --without-sqlite

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_SQLITE3)),y)
...
ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_SSL)),y)
...
$(PKG)_SYSVIPC_SUPPORT:=$(if $(FREETZ_PACKAGE_PHP5_WITH_SYSVIPC),enable,disable)
...
ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_ZLIB)),y)
$(PKG)_DEPENDS_ON += zlib
$(PKG)_CONFIGURE_OPTIONS += --with-zlib
$(PKG)_CONFIGURE_OPTIONS += --with-zlib-dir="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr"
endif

ifeq ($(strip $(FREETZ_PACKAGE_PHP5_WITH_ZIP)),y)
$(PKG)_CONFIGURE_OPTIONS += --enable-zip
endif

$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_STATIC
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_apxs2
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_CURL
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_FILEINFO
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_FTP
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_GD
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_GETTEXT
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_ICONV FREETZ_PACKAGE_PHP5_WITH_LIBICONV
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_JSON
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_LIBXML FREETZ_LIB_libxml2_WITH_HTML
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_MEMORY_LIMIT
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_MHASH
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_PCNTL
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_SESSION
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_SOCKETS
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_SQLITE3
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_SYSVIPC
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_SSL
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_PHP5_WITH_ZLIB
...
$($(PKG)_BINARY) $($(PKG)_CLI_BINARY) $($(PKG)_APXS2_BINARY): $($(PKG)_DIR)/.configured
	$(SUBMAKE) -C $(PHP5_DIR) \
		EXTRA_CFLAGS="$(PHP5_EXTRA_CFLAGS)" \
		EXTRA_LDFLAGS_PROGRAM="$(PHP5_EXTRA_LDFLAGS)" \
		ZEND_EXTRA_LIBS="$(PHP5_LIBS)"
...
$(pkg)-clean:
	-$(SUBMAKE) -C $(PHP5_DIR) clean
	$(RM) $(PHP5_FREETZ_CONFIG_FILE)

$(pkg)-uninstall:
	$(RM) $(PHP5_TARGET_BINARY) $(PHP5_CLI_TARGET_BINARY) $(PHP5_APXS2_TARGET_BINARY)
