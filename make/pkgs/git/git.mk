$(call PKG_INIT_BIN, $(if $(FREETZ_PACKAGE_GIT_VERSION_ABANDON),2.26.2,2.55.0))
$(PKG)_SOURCE:=$(pkg)-$($(PKG)_VERSION).tar.xz
$(PKG)_HASH_ABANDON:=6d65132471df9e531807cb2746f8be317e22a343b9385bbe11c9ce7f0d2fc848
$(PKG)_HASH_CURRENT:=457fdb04dc8728e007d4688695e6912e6f680727920f2a40bf11eacc17505357
...
$(PKG)_DEPENDS_ON += $(if $(FREETZ_TARGET_UCLIBC_0_9_28),iconv)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_GIT_WITH_PCRE),pcre)
$(PKG)_DEPENDS_ON += $(if $(FREETZ_PACKAGE_GIT_WITH_PCRE2),pcre2)

$(PKG)_EXCLUDED+=$(if $(FREETZ_PACKAGE_GIT_REMOVE_WEBIF),etc/default.git etc/init.d/rc.git usr/lib/cgi-bin/git.cgi)

$(PKG)_CONDITIONAL_PATCHES+=$(if $(FREETZ_PACKAGE_GIT_VERSION_ABANDON),abandon,current)

$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_GIT_VERSION_ABANDON
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_GIT_WITH_PCRE
$(PKG)_REBUILD_SUBOPTS += FREETZ_PACKAGE_GIT_WITH_PCRE2
...
$(PKG)_CONFIGURE_OPTIONS += --with-libpcre1="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr"
endif
ifeq ($(strip $(FREETZ_PACKAGE_GIT_WITH_PCRE2)),y)
$(PKG)_CONFIGURE_OPTIONS += --with-libpcre2="$(TARGET_TOOLCHAIN_STAGING_DIR)/usr"
endif
